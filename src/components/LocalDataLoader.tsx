...
export function LocalDataLoader({ onLoadData }: LocalDataLoaderProps) {
  ...
  const loadLocalData = async () => {
    ...
    try {
      // Load the local canvas data
      const response = await fetch('/local_canvas_data.json');
      const data = await response.json();
      
      if (data && data.items) {
        onLoadData(data);
        setLoadedItems(data.items.length);
        console.log(`✅ Loaded ${data.items.length} items from local data`);
      } else {
        console.error('❌ No data found in local_canvas_data.json');
      }
    } catch (error) {
  ...
  const loadMockData = () => {
    ...
    const mockData = {
      ...
      "lastAction": "loaded_from_local_data",
      "itemsCreated": 12,
      "syncSheetId": "",
      "syncSheetName": ""
    };
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle>📊 Load Local Data</CardTitle>
        <CardDescription>
          Load supply chain data directly from local CSV files
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Button 
            ...
            className="w-full"
          >
            {isLoading ? "Loading..." : "📁 Load from JSON File"}
          </Button>
        ...
        <div className="text-xs text-gray-500">
          <p><strong>Option 1:</strong> Load from local_canvas_data.json</p>
          <p><strong>Option 2:</strong> Load embedded mock data</p>
        </div>
      </CardContent>
    </Card>
  );
